...
public $endTS = 0;
     ...
     * @param int       $endTS
     */
    public function __construct($startTS = 0, $endTS = 0)
    {
    ...
    {
        $startTS       = (int)$this->startTS;
        $startTS       = ($this->startTS > 0) ? $this->startTS : time();
        $startDatetime = getdate($this->startTS);

        $endTS       = (int)$this->endTS;
        $endTS       = ($this->endTS > 0) ? $this->endTS : time();
        $endDatetime = getdate($this->endTS);

        $form = new \XoopsFormElementTray("dateDebEnd", '<br>');
        ...
        //=========================================
        // End date element's form
        $endElmtTray     = new \XoopsFormElementTray(_MD_EXTCAL_EVENT_END, '<br>');
        $endElmtTray->addElement(new \XoopsFormRadioYN('', 'have_end', 1));
        $form->addElement($endElmtTray);

        //=========================================
        // End date element's form
        $endDateElmtTray = new \XoopsFormElementTray(_MD_EXTCAL_END_DATE, '&nbsp;');
        ...
        $form->addElement($endDateElmtTray);
        //=========================================
        return $form->render();
    }
}
