...
//    $xoopsTpl->assign('xoops_pagetitle', _MI_EXTCAL_SUBMIT_EVENT);
    ...
    //exit;
        ...
        $data['event_id'] = $eventHandler->getInsertId();
        $fileHandler->createFile($data['event_id'] );
    $data['url_notification_event'] = XOOPS_URL . "/modules/extcal/event.php?event=" . $data['event_id'];
    $data['url_notification_cat'] = XOOPS_URL . "/modules/extcal/view_calendar-month.php?cat=" . $data['cat_id']  ;
        ...
        //$notificationHandler->triggerEvent('global', 0, $notifyEvent, ['EVENT_TITLE' => $_POST['event_title'], 'CAT_NAME'=>$cat->getVar('cat_name')]);
        $notificationHandler->triggerEvent('global', 0, $notifyEvent, $data);

        if ($approve == 1) {
            //            $catHandler = xoops_getModuleHandler(_EXTCAL_CLS_CAT, _EXTCAL_MODULE);
            $notificationHandler->triggerEvent('cat', \Xmf\Request::getInt('cat_id', 0, 'POST'), 'new_event_cat', [
                'EVENT_TITLE' => $_POST['event_title'],
                'CAT_NAME'    => $cat->getVar('cat_name'), //."|test notification => bbbbbbbbbbbbbbbbbb|"
            ]);
        }
    }
    if ($approve) {
