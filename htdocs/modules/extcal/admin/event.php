...
if (!isset($op))  $op = '';
if (!isset($sel_approved)) $sel_approved = -1;
if (!isset($sel_cat)) $sel_cat = 0;
// echo "op = {$op}<br>";
// echo "sel_approved = {$sel_approved}<br>";
// echo "sel_cat = {$sel_cat}<br>";
//--------------------------------------------------------------------
/**
 * @param $ids
 *
...
switch ($op) {
    ...
    $data['cat_name'] = $cat->getVar('cat_name');

        // Event edited
        if (\Xmf\Request::hasVar('event_id', 'POST')) {
            if (!$eventHandler->modifyEvent($_POST['event_id'], $data)) {
                redirect_header("event.php?op=list&sel_approved={$sel_approved}&sel_cat={$sel_cat}", 3, _AM_EXTCAL_EVENT_EDIT_FAILED, false);
            } else {
                $fileHandler->createFile(\Xmf\Request::getInt('event_id', 0, 'POST'));
                redirect_header("event.php?op=list&sel_approved={$sel_approved}&sel_cat={$sel_cat}", 3, _AM_EXTCAL_EVENT_EDITED, false);
            }
...
//exit ("approved = {$approved}");
            if ($eventHandler->createEvent($data, $_POST)) {
                ...
                $notificationHandler->triggerEvent('global', 0, 'new_event', [
                    'EVENT_TITLE' => $_POST['event_title'],
                    'CAT_NAME'    => $cat->getVar('cat_name') //."|test notification => ooooooooooooooooooooooo|"
                  ]);
                //$notificationHandler->triggerEvent('global', 0, 'new_event', [$data);

                $notificationHandler->triggerEvent('category', $_POST['cat_id'], 'new_event_cat', [
                    'EVENT_TITLE' => $_POST['event_title'],
                    'CAT_NAME'    => $cat->getVar('cat_name'), //."|test notification => vvvvvvvvvvvvvvvvv|"
                ]);
                redirect_header("event.php?op=list&sel_approved={$sel_approved}&sel_cat={$sel_cat}", 3, _AM_EXTCAL_EVENT_CREATED, false);
            } else {
                redirect_header("event.php?op=list&sel_approved={$sel_approved}&sel_cat={$sel_cat}", 3, _AM_EXTCAL_EVENT_CREATE_FAILED, false);
            }
        }
        break;
    //--------------------------------------------------------------
    ...
    case 'modify':
        ...
        if ($form = $eventHandler->getEventForm('admin', $action, ['event_id' => $eventId, 'sel_approved' => $sel_approved,  'sel_cat' => $sel_cat])) {
            $form->display();
        }
    ...
    //----------------------------------------------------------
    case 'bascule_approved':

        $eventId      = $_GET['event_id'];
        $eventHandler = Extcal\Helper::getInstance()->getHandler(_EXTCAL_CLN_EVENT);
        $event        = $eventHandler->getEvent($eventId, false,-1);
        $event->setVar('event_approved', ! $event->getVar('event_approved'));
        $eventHandler->insert($event, true);

        redirect_header("event.php?op=list&sel_approved={$sel_approved}&sel_cat={$sel_cat}", 3, _AM_EXTCAL_EVENT_UPDATED, false);
    ...
    default:
        ...
        $events       = $eventHandler->objectToArray($eventHandler->getNewEvent($start, $nbEventsByPage, $sel_cat, true, $sel_approved, 'event_start', 'DESC'), ['cat_id', 'event_submitter']);
//ext_echo($events);

        $eventHandler->formatEventsDate($events, _SHORTDATESTRING);
/* la légende n'est pas utile puisqu'il y a les "title" sur les icones
...
*/
        ...
        echo '<th>' . _AM_EXTCAL_SUBMITER . '</th>';
        ...
        if (count($events) > 0) {
            ...
            foreach ($events as $event) {
                $param = "&event_id={$event['event_id']}&sel_cat={$sel_cat}&sel_approved={$sel_approved}";
                ...
                echo '<td align="center">' . ((1 == $event['event_approved']) ? _YES : _NO). "&nbsp;&nbsp;&nbsp;&nbsp;";
                if (1 == $event['event_approved']){
                  echo '<a href=event.php?op=bascule_approved' . $param . "><img src='" . $pathIcon16 . "/0.png' title='" . _AM_EXTCAL_INACTIVATE_EVENT . "'></a>";
                }else{
                  echo '<a href=event.php?op=bascule_approved' . $param . "><img src='" . $pathIcon16 . "/1.png' title='" . _AM_EXTCAL_APPROVE_EVENT . "'></a>";
                }
                echo '</td>';

                echo '<td style="width:10%; text-align:center;">';

                echo '<a href=event.php?op=modify' . $param . "><img src='" . $pathIcon16 . "/edit.png' title='" . _AM_EXTCAL_ICONE_EDIT . "'></a>&nbsp;&nbsp;";
                echo '<a href=event.php?op=delete' . $param . "><img src='" . $pathIcon16 . "/delete.png' title='" . _AM_EXTCAL_ICONE_DELETE . "'></a>&nbsp;&nbsp;";
                echo '<a href=event.php?op=clone' . $param . "><img src='" . $pathIcon16 . "/editcopy.png' title='" . _AM_EXTCAL_ICONE_CLONE . "'></a>";
                echo '</td>';
            ...
            if ($sel_cat > 0) $criteria->add(new \Criteria('cat_id', $sel_cat));
            if ($sel_approved >= 0) $criteria->add(new \Criteria('event_approved', $sel_approved));

            $nbEvents = $eventHandler->getCount($criteria);

            $pageNav = new \XoopsPageNav($nbEvents, $nbEventsByPage, $start);
            $pageNav->url = XOOPS_URL . "/modules/extcal/admin/event.php?sel_cat={$sel_cat}&sel_approved={$sel_approved}&start=";
            echo '<tr><td colspan="2" style="text-align: right;">';
            echo $pageNav->renderNav(2);
//             echo "<br>===>" . $pageNav->url;
//             echo "<br>===>nb events = " . $nbEvents  ;
//             echo "<br>===> = sel_cat = " .   $sel_cat;
//             echo "<br>===> sel_approved = " .   $sel_approved;
            echo '</td>';
