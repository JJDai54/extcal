...
trait VersionChecks
{
    ...
    public static function checkVerXoops(\XoopsModule $module = null, $requiredVer = null)
    {
        ...
        if (version_compare($currentVer, $requiredVer, '<')) {
            $success = false;
            $module->setErrors(sprintf(constant('_CO_' . $moduleDirNameUpper . '_ERROR_BAD_XOOPS'), $requiredVer, $currentVer));
        }
    ...
    public static function checkVerPhp(\XoopsModule $module = null)
    {
        ...
        if (false !== $reqVer && '' !== $reqVer) {
            if (version_compare($verNum, $reqVer, '<')) {
                $module->setErrors(sprintf(constant('_CO_' . $moduleDirNameUpper . '_ERROR_BAD_PHP'), $reqVer, $verNum));
                $success = false;
            }
        }
     ...
     * @static
     * @param \Xmf\Module\Helper $extcalHelper
    ...
    public static function checkVerModule($extcalHelper, $source = 'github', $default = 'master')
    {
        $moduleDirName      = basename(dirname(dirname(__DIR__)));
        $moduleDirNameUpper = mb_strtoupper($moduleDirName);
        $update             = '';
            ...
            if (function_exists('curl_init') && false !== ($curlHandle = curl_init())) {
                ...
                if (false === $curlReturn) {
                    ...
                    if ('master' !== $latestVersionLink) {
                        $update = constant('_CO_' . $moduleDirNameUpper . '_' . 'NEW_VERSION') . $latestVersion;
                    }
                    ...
                    $moduleVersion = ($extcalHelper->getModule()->getInfo('version') . '_' . $extcalHelper->getModule()->getInfo('module_status'));
                    //"PHP-standardized" version
