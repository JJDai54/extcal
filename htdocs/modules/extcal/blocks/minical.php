...
function bExtcalMinicalShow($options)
{
    ...
    setlocale (LC_TIME, _MB_EXTCAL_LOCALISATION); //'en_EN''fr_FR'
    $weekdayNames = Calendar_Util_Textual::weekdayNames('two');
    ...
    $ret = [
        ...
        'submitText'   => _MB_EXTCAL_SUBMIT_LINK,
        'tableRows'    => $tableRows,
...
//ext_echo($options);
  $ret['saint']['show'] =  intval($options[14]);
  if ($options[14] == 1){
    //echo $saintHandler->getSaints(date("n"), date("j")) . "<br>";
    $ret['saint']['name'] = $saintHandler->getHTML(date("n"), date("j"), _MB_EXTCAL_FESTE_OF_DAY, $attributs);
  }
// ----------------------------------
...
function bExtcalMinicalEdit($options)
{
    ...
    //---------------------------------------------------------------------
    $k           = 14;
    $xfValue[$k] = new \XoopsFormRadio(_MB_EXTCAL_DISPLAY_SAINT, "options[{$k}]", $options[$k]);
    $xfValue[$k]->addOption(1, _YES);
    $xfValue[$k]->addOption(0, _NO);
