...
*}>
<script src='<{$smarty.const.XOOPS_URL}>/Frameworks/highslide/xoops_highslide.js' type="text/javascript"></script>
...
<{if $location.location_id.value != 0}>
    <table border="0" width="100%" class="outer">
        <tr>
            <td width="_EXTCAL_TS_MINUTE%" class="odd">
                ...
                <a href="./location.php?location_id=<{$event.location_id}>">
                ...
                <{if $location.logo.value}>
                    <div class="highslide-gallery">
                        <a href="<{$xoops_url}>/uploads/extcal/location/<{$location.logo.value}>"
                           class="highslide" onclick="return hs.expand(this, {maxWidth:600})">
                            <img align="left" style="margin-right:10px;"
                                 src="<{$xoops_url}>/uploads/extcal/location/thumbs/<{$location.logo.value}>"
                                 height="150px">
                        </a>
...
<table class="outer woosgoing"  width="100%">

    <{if $event.event_nbmember > 0}>
        <tr>
            <td colspan="3" class="even">
                <b><{$smarty.const._MD_EXTCAL_NBMEMBER}> : <{$event.event_nbmember}>
            </td>
        </tr>
    <{if $whosGoing}>
        <tr class="even woosgoing" >
            <td class="even woosgoing" width="180px">
                <b><{$smarty.const._MD_EXTCAL_WHOS_GOING}> (<{$eventmember.member.nbUser}>) :</b>
            </td>
            <td class="odden woosgoing" width="100px">
            ...
            <td class="even woosgoing" style="padding-left:10px;padding-right:10px;">
                    <{foreach name=eventMemberList from=$eventmember.member.userList item=member}>
                      <{if $smarty.foreach.eventMemberList.first != 1}>, <{/if}>
    ...
    <{if $whosNotGoing}>
        <tr class="even woosgoing" >
            <td class="even woosgoing" width="180px">
                <b><{$smarty.const._MD_EXTCAL_WHOSNOT_GOING}> (<{$eventmember.notmember.nbUser}>) :</b>
            </td>
            <td class="odden woosgoing" width="100px">
            ...
            <td class="even woosgoing" style="padding-left:10px;padding-right:10px;">
                <{foreach name=eventMemberList from=$eventmember.notmember.userList item=member}>
                      <{if $smarty.foreach.eventMemberList.first != 1}>, <{/if}>
                    <a href="<{$xoops_url}>/userinfo.php?uid=<{$member.uid}>"><{$member.uname}></a>
                <{/foreach}>
            </td>
